export const Pagination = ({ activePage, count, rowsPerPage, totalPages, setActivePage }) => {
    const start = activePage === 1 ? 1 : rowsPerPage * (activePage - 1) + 1
    const end = activePage === totalPages ? count : start + rowsPerPage - 1
  
    return (
      <>
        <div className="pagination">
					<p>
						Rows: {start === end ? end : `${start} - ${end}`} of {count} ( Page {activePage} of {totalPages} )
        	</p>
